...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/flawed-portability-test.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
...
test_marker_removal() (
    FLAWED_SCRIPT_LIB_ONLY=1 source "$ROOT/scripts/uninstall.sh"
    local dir="$TMP/markers"
    mkdir -p "$dir"

    printf '%s\n' before '# Flawed-Shell begin' managed '# Flawed-Shell end' after > "$dir/valid.conf"
    _remove_block "$dir/valid.conf" '# Flawed-Shell begin' '# Flawed-Shell end' \
    ...
    for name in missing-end reversed duplicate-begin duplicate-pair; do
        case "$name" in
            missing-end)
                printf '%s\n' before '# Flawed-Shell begin' valuable > "$dir/$name.conf"
                ;;
            reversed)
                printf '%s\n' before '# Flawed-Shell end' middle '# Flawed-Shell begin' valuable > "$dir/$name.conf"
                ;;
            duplicate-begin)
                printf '%s\n' before '# Flawed-Shell begin' one '# Flawed-Shell end' middle '# Flawed-Shell begin' valuable > "$dir/$name.conf"
                ;;
            duplicate-pair)
                printf '%s\n' before '# Flawed-Shell begin' one '# Flawed-Shell end' middle '# Flawed-Shell begin' two '# Flawed-Shell end' valuable > "$dir/$name.conf"
                ;;
        esac
        cp "$dir/$name.conf" "$dir/$name.before"
        if _remove_block "$dir/$name.conf" '# Flawed-Shell begin' '# Flawed-Shell end'; then
            fail "$name markers were accepted"
        fi
        [ "$(<"$dir/$name.before")" = "$(<"$dir/$name.conf")" ] || fail "$name markers changed the file"
    done

    printf '%s\n' before '-- Flawed-Shell begin' managed '-- Flawed-Shell end' after > "$dir/target.lua"
    ln -s target.lua "$dir/link.lua"
    _remove_block "$dir/link.lua" '-- Flawed-Shell begin' '-- Flawed-Shell end' \
...
test_qml_module_lookup() (
    ...
    mkdir -p "$imports/Flawed/TestModule"
    printf 'module Flawed.TestModule\n' > "$imports/Flawed/TestModule/qmldir"
    ...
    QML_IMPORT_PATH=""
    FLAWED_SCRIPT_LIB_ONLY=1 source "$ROOT/scripts/install.sh"

    _qml_module_available Flawed.TestModule \
        || fail "QML module in temporary import root was not found"
    if _qml_module_available Flawed.AbsentModule; then
...
test_hypr_discovery() {
    ...
    local actual
    actual="$(
        FLAWED_PROC_ROOT="$proc" FLAWED_PARENT_PID=200 \
    ...
    actual="$(
        FLAWED_PROC_ROOT="$unique" FLAWED_PARENT_PID=999 \
        HOME="$TMP/home" XDG_CONFIG_HOME="$TMP/home/config" \
        bash "$ROOT/scripts/install.sh" --hypr-config-path
    )"
    ...
    actual="$(
        FLAWED_PROC_ROOT="$ambiguous" FLAWED_PARENT_PID=999 \
    ...
    actual="$(
        FLAWED_PROC_ROOT="$empty" FLAWED_PARENT_PID=999 \
...
test_atomic_units() (
    FLAWED_SCRIPT_LIB_ONLY=1 source "$ROOT/scripts/update.sh"
    ...
    if find "$SYSTEMD_USER_DIR" -maxdepth 1 -name '.flawed-update.*.??????' -print -quit | grep -q .; then
